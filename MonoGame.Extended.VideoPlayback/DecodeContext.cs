...
using BytingLib.Framework.Media;
using BytingLib.VideoPlayback.AudioDecoding;
using BytingLib.VideoPlayback.Extensions;
using BytingLib.VideoPlayback.VideoDecoding;
using Sdcb.FFmpeg.Raw;
using System.Threading.Tasks;

namespace BytingLib.VideoPlayback;
    ...
    internal void ReadVideoUntilPlaybackIsAfter(double presentationTime)
    {
        ...
        if (!r && videoPackets.Count == 0)
        {
            ...
            else
            {
                ...
                {
                    // Must invoke on new thread. See the explanations of Ended.
                    var handler = Ended;
                    if (handler != null)
                    {
